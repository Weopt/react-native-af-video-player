...
import { Text, StyleSheet, StatusBar, Dimensions, BackHandler, Animated, Image, Alert } from 'react-native';
...
class Video extends Component {
  ...
  renderError() {
    ...
    return (
      <Animated.View
        style={[
          styles.background,
          fullScreen ? (styles.fullScreen, { height: this.animFullscreen }) : { height: this.animInline },
  ...
  renderPlayer() {
    const {
      fullScreen, paused, muted, loading, progress, duration, inlineHeight, currentTime,
    } = this.state;
    ...
    return (
      ...
      >
        ...
        {((loading && placeholder) || currentTime < 0.01) && <Image resizeMode="cover" style={styles.image} {...checkSource(placeholder)} />}
        <VideoPlayer
          {...checkSource(url)}
          paused={paused}
